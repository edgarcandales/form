...
interface IRNPickerSelectProps {
  ...
  touchableDoneProps: { [key: string]: any };
  touchableWrapperProps: { [key: string]: any };
  Icon?: React.FC<{ testID: string }>;
  InputAccessoryView: React.FC<{ testID: string }>;

  iconContainer: any;
}

interface IRNPickerSelectState {
  items: IItem[];
  selectedItem: IItem;
  showPicker: boolean;
...
export default class RNPickerSelect extends PureComponent<IRNPickerSelectProps, IRNPickerSelectState> {
  static propTypes = {
  ...
  static handlePlaceholder({placeholder}: { placeholder: IItem }) {
  ...
  static getSelectedItem({items, key, value}: { items: IItem[], key: string | number, value: any }) {
    let idx = items.findIndex((item: IItem) => {
      if (item.key && key) {
  ...
  constructor(props: IRNPickerSelectProps) {
    super(props);
  ...
  componentDidUpdate = (prevProps: IRNPickerSelectProps, prevState: IRNPickerSelectState) => {
  ...
  setInputRef(ref: React.RefObject<any>) {
    this.inputRef = ref;
  }
  ...
  togglePicker(animate = false, postToggleCallback?: () => void) {
